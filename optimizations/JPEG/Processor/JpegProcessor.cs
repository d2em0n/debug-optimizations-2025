...
public class JpegProcessor : IJpegProcessor
{
    ...
    private static float[,] _deQuantizedBuffer = new float[DCTSize, DCTSize];
    ...
    private static CompressedImage Compress(Matrix matrix, int quality = 50)
    {
        ...
        var quantizedBytesList = new List<byte>[height/DCTSize];

        Parallel.For(0, height / DCTSize, (y) =>
        {
            ...
            for (var x = 0; x < width; x += DCTSize)
            {
                ...
                {
                    var subMatrix = GetSubMatrix(matrix, y * DCTSize, DCTSize, x, DCTSize, selector);
                    ...
                    locBytes.AddRange(quantizedBytes);
                }
            }
            quantizedBytesList[y] = locBytes;
        });
        for(var y = 0; y < height/DCTSize; y++)
        {
            allQuantizedBytes.AddRange(quantizedBytesList[y]);
        }
    ...
    private static ArrayPool<float> channelPool = new ArrayPool<float>(DCTSize, DCTSize);
    ...
    private static Matrix Uncompress(CompressedImage image)
    {
               ...
               new MemoryStream(HuffmanCodec.Decode(image.CompressedBytes, image.DecodeTable, image.BitsCount)))
        {
            for (var y = 0; y < height; y += DCTSize)
            {
                for (var x = 0; x < width; x += DCTSize)
                {
                    var _y = channelPool.Rent();
                    var cb = channelPool.Rent();
                    var cr = channelPool.Rent();
                    ...
                    channelPool.Return(_y);
                    channelPool.Return(cb);
                    channelPool.Return(cr);
                }
            }
        }
    ...
    {
        var _quantizedBuffer = new byte[DCTSize, DCTSize];
